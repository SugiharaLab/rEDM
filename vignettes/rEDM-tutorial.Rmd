...
To identify convergence, we compute cross map skill over many random subsamples of the time series. The `lib_sizes` argument specifies the size of the library set, and `num_samples` specifies the number of subsamples generated at each library size. `random_libs` and `replace` specify how the subsamples will be generated. Here, setting both to `TRUE` enables random sampling with replacement.
...
Our next step is to produce forecast models that combine several different variables. In order to use `block_lnlp`, we first need to construct lags for each variable. This requires a bit of care in coding, as we need to ensure that lagged components come only from observations within a single field and transect.
...
Next, we run `block_lnlp` to compare predictions for primary productivity using either its own lagged dynamics, or when including information about precipitation:
...
We examine the model performance by comparing predicted vs. observed values:
...
There is a slight improvement in forecast skill, and the data are more closesly associated on the 1:1 line. However, there is still a great deal of variability in biomass that is unexplained.
