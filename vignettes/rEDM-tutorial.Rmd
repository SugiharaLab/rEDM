...
Note that for two variables (nitrogen and species richness), the best embedding dimension is also the maximum that we test. These results suggest that the dynamics might be high-dimensional, and that collecting longer time series might enable better predictions.
...
Note that species richness and precipitation suggest nonlinear dynamics in the data (because of the initial increase in forecast skill for `theta > 0`, followed by a sharp drop-off). However, the monotonic decrease in `rho` for `theta > 0` in the case of `noh020tot` and `AbvBioAnnProd` suggest that soil nitrate levels and total biomass may have only linear dynamics.

### Multivariate Models

Our next step is to produce forecast models, and specifically to compare models that predict total biomass using only biomass in the past with models that also include total nitrate. In the package, we can do this using `block_lnlp`, but first we need to construct lags for each variable. 

Here, we use the `make_block` function, which automatically adds lags of the variables. However, we also need to be careful, since the raw data combines observations from multiple fields and transects. By including the `lib` argument, we can specify where the time series segments are, so that missing data is properly recorded.
...
Next, we run `block_lnlp`, being sure to setup the `columns` argument to include the variables that we want for each model, as well as the variable to be predicted (by default, predictions are always for one step ahead):
...
There is a slight improvement in forecast skill when including nitrate as a predictor, and the data are more closesly associated on the 1:1 line. However, there is still a great deal of variability in biomass that is unexplained.
