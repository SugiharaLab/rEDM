...
The remaining arguments will be left at their default values (see section \ref{sec_general_parameters} for details). For the `simplex` function, this means that the embedding dimension, $E$, will range from $1$ to $10$.
...
In contrast to the nearest-neighbor interpolation of simplex projection, the S-map forecasting method [@Sugihara_1994] fits local linear maps to forecast from the reconstructed state space. Here, "local" means that the map is different at different locations in the reconstructed state space.

With S-maps, there is an additional nonlinear tuning parameter, $\theta$, that determines the strength of the weighting when fitting the lcoal linear map. For example, when $\theta = 0$, all points always receive equal weighting -- thus the local linear map does not change, and will be identical to a global linear map, equivalent to fitting an autoregressive model to the data. When $\theta > 0$, nearby points receive larger weights, allowing the local linear map to vary in state-space and accommodate nonlinear behavior (see the vignette "rEDM-algorithms" for more details).

Consequently, if the time series are sampled from autoregressive red noise, then the linear model ($\theta = 0$) will produce better forecasts by including all the points to reduce the effects of observational noise. In contrast, increases in forecast skill when $\theta > 0$ is indicative of nonlinear dynamics, because allowing the local linear map to vary better describes state-dependent behavior.

### Example

The S-map method is implemented as the function `s_map` in the **rEDM** package. Following from the previous example, we again use the tent map time series, and set `E = 2` based on the results from simplex projection. Note that we allow many of the arguments to take on default values (e.g., `\tau = 1`, `tp = 1`). If we had changed these for simplex projection, we would want to propagate them here. The default values for the nonlinear tuning parameter, `theta`, range from `0` to `8`, and are suitable for our purposes.

Note also, that the default value for `num_neighbors` is `0`. When using `s_map` to test for nonlinear behavior, we want to use all points in the reconstruction, and allow `theta` to control the weighting assigned to individual points. By using `0` for this argument (an otherwise nonsensical value), the program will use all nearest neighbors.
...
Instead of creating an attractor using lags of one time series, it is possible to combine different time series, if they are all observed from the same system [@Sauer_1991; @Deyle_2011]. The practical reality of dealing with complex dynamics, finite data, noisy data, and stochastic drivers means that "multivariate" reconstructions can often be a better description than "univariate" reconstructions.

In **rEDM**, the `block_lnlp` function generalizes the `simplex` and `s_map` functions, allowing reconstructions to have any set of coordinates, and with forecasting via either of the two methods.

The input format for `block_lnlp` is a matrix or data.frame of the time series observations, where each column is a separate time series with rows representing "simultaneous" observations. A reconstruction is defined by specifying which columns to use as coordinates (the `columns` argument) and which column is to be forecast (the `target_column` argument). Here, if lagged copies of a time series are desired as coordinates, they need to be separate columns in the input.
...
Here, the `block_3sp` variable is a 10-column data.frame with 1 column for time, and 3 columns for each of the variables (unlagged: $t$, lag-1: $t-1$, and lag-2: $t-2$). Note that the lagged columns begin with `NA` values because there are no observations of the variables for times $t < 1$. In **rEDM**, vectors that include `NA` values are excluded if they would be used for computation, but predictions will still be made if only the observed values are missing (see section \ref{sec_data_input} for more details).

In order to correctly index into columns, `block_lnlp` has an option to indicate that the first column is actually a time index. When `first_column_time` is set to `TRUE`, a value of `1` for `target_column` will point to the first *data* column in the data.frame.
...
We can also refer to columns by the column names, which ignores the `first_column_time` argument:
...
To compare the observed and predicted values, we want to pull out the appropriate columns of the data.frame for the 1 model that we ran:
```{r}
...
One of the corollaries to Takens' Theorem is that multiple reconstructions should not only map to the original system, but also to each other. Consider two variables, $x$ and $y$ that interact in a dynamic system. Then the respective univariate reconstructions based on lags of $x$ ($\mathbf{M}_x$) or $y$ ($\mathbf{M}_y$) should uniquely identify system states. Moreover, the reconstructed states of $\mathbf{M}_x$ should map to reconstructed states of $\mathbf{M}_y$.
...
This suggests a way to test whether $x$ and $y$ interact in the same system, by testing for a mapping between $\mathbf{M}_x$ and $\mathbf{M}_y$. Furthermore, in the case of unidirectional causality, e.g. $x$ causes $y$, we would only expect cross mapping to be successful in one direction. Somewhat counterintuitively, if a causal variable ($x$) leaves a signature on the affected variable ($y$), then it is possible to map from $\mathbf{M}_y$ to $x$, but not vice-versa.

In essence, because $\mathbf{M}_y$ must have complete information about $y$, it must include information about all its causes, which includes $x$. In contrast, because $x$ behaves independently of $y$, its reconstruction, $\mathbf{M}_x$, is missing information about $y$, and cross mapping will be incomplete.

Although $x$ has incomplete information about $y$, it does affect the values of $y$, and will likely have some predictive skill. However, this cross mapping will be limited to the statistical association between $x$ and $y$ and will generally not improve with more data. In contrast, $\mathbf{M}_y$ is expected to become complete with more data, and the mapping to $x$ will improve. This convergence is a critical property for inferring causality, and can be tested by measuring the cross mapping skill when using different amounts of data to reconstruct $\mathbf{M}_y$. For a more detailed description of using cross mapping to infer causation, see [@Sugihara_2012, @Ye_2015a].

## Convergent Cross Mapping (CCM)

In **rEDM**, convergent cross mapping is implemented as the `ccm` function, which provides a convenient wrapper to compute cross map skill for different subsamples of the data. In the following example, we reproduce the analysis from [@Sugihara_2012] to identify causality between anchovy landings in California and Newport Pier sea-surface temperature. For this example, we use the previously identified embedding dimension of `E = 3`.
...
However, `tp` is an argument to `ccm`, and will accept both positive and negative values. This allows us to determine the value of `tp` that produces the best mapping for $F$. This is, to a first approximation, the time delay by which information about $y$ is encoded in the time series of $x$ [see @Ye_2015a for more information].

Note that negative values of `tp` ($tp < 0$) indicate that *past* values of $x$ are mapped to from the reconstructed states of $\mathbf{y}$. This suggests that the dynamical signal that is detected appears first in $x$ and later in $y$, and is consistent with $x$ causing $y$.
...
would be best at a positive value of `tp` ($tp > 0$).
...
We can determine the optimal values of `tp` by testing different values with the `ccm` function. We do this by using as much data as possible, in order to obtain the cleanest signal:
...
The columns in the dataset `e120_invnit16` are as follows: `Exp` indicates the experiment code (E120), `Year` shows the sampling time; `Plot` describes plot identity; `AbvBioAnnProd` shows annual above-ground productivity of planted species, in g/m$^2$; `noh020tot` shows soil nitrate levels in the top 20 cm of soil, measured in $\mu$g/kg soil; `invrichness` shows species richness of unplanted species in the plot (i.e. weeds); `SummerPrecip.mm.` shows precipitation annual from May to August measured in mm.
...
To prevent lagged vectors from being constructed that span separate plots, we need to create an appropriate index variable that identifies different segments. The accuracy of EDM predictions can then be assessed through cross-validation by selecting a random subset of plots to use for the library and prediction sets.
...
Note, to successfully apply EDM, we should include only **unique observations**. Because the time series for precipitation does not vary among replicates (i.e. it is the same annual time series for each plot), we also need to construct separate variables for analyzing precipitation dynamics, with only one observation per year:
...
simplex_out[[4]] <- simplex(precip_ts, E = 2:5, silent = TRUE)
...
Note that for two variables (aboveground biomass and invader richness), the best embedding dimension is also the maximum that we test. These results suggest that the dynamics might be high-dimensional, and that collecting longer time series could enable better predictions and a cleaner signal to be detected by our analyses.
...
Note that aboveground biomass, invader richness, and precipitation suggest nonlinear dynamics in the data (because of the initial increase in forecast skill for `theta > 0`, followed by a gradual drop-off). However, the monotonic decrease in `rho` for `theta > 0` in the case of `noh020tot` suggests that soil nitrate levels may have only linear dynamics.

### Multivariate Models

Our next step is to produce forecast models. Recall that several variables were best predicted using the maximum number of available time lags. As such, it could be that adding additional information from other variables might improve predictions. Specifically, we will compare models that predict aboveground biomass using only time lagged observations of biomass with models that also include precipitation. In the package, we can do this using `block_lnlp`, but first we need to construct lags for each variable. 

Here, we use the `make_block` function, which automatically adds lags of the variables. However, we also need to be careful, since the raw data combines observations from multiple plots. By including the `lib` argument, we can specify where the time series segments are, so that missing data is properly recorded.
...
Next, we run `block_lnlp`, being sure to set up the `columns` argument to include the variables that we want for each model, as well as the variable to be predicted (by default, predictions are always for one step ahead). Note, we include one run using only lagged observations of biomass, and one run that also includes precipitation:
...
We can then examine model performance by comparing predicted vs. observed values:
...
There is a marked improvement in forecast skill when including precipitation as a predictor, and the data are more closely associated on the 1:1 line. However, there is still a great deal of variability in biomass that is unexplained ($R^2 = 0.48$).
...
Recall, we are primarily interested in testing whether the biomass of planted species or soil nitrogen concentration causally forces the dynamics of invading species.
...
These results suggest that invader richness is likely causally forced by the biomass of the planted community, but that nitrate dynamics may be more strongly forced by invader dynamics than vice-versa. In contrast to existing hypotheses, this result could potentially suggest that at least in plots planted with 16 species, planted community biomass itself directly inhibits invasion (e.g. through light limitation), rather than through indirect effects of planted biomass on soil nitrate. 

The first figure suggests that plot-level soil nitrate is causally forced by invader richness, while the reverse is not true. This is plausible, as the planted compositions are quite similar among experimental replicates, and variations in soil nitrate dynamics may be largely indicative of minor variations due to transient populations of invading species before they can be weeded out. However, recall from the S-mapping analyses that soil nitrate dynamics are likely largely linear. As such, it may be a good idea to apply other methods for testing causal forcing that are more suitable for linear systems, such as Granger Causality [.

Our analyses for biomass and invasion may indicate either bidirectional forcing, or synchrony. However, there is a much stronger rise in $\rho$ with library length for "Inv. Richness xmap Total Biomass" (i.e., "Biomass affects Invasive Richness") than for the reverse direction, and $\rho$ is generally small, suggesting that synchrony is unlikely. Furthermore, there does not appear to be a much difference between $\rho$ at short library lengths and $\rho$ at large library lengths for "Total Biomass xmap Inv. Richness", suggesting that this result may be spurious. One other potential caveat is that for both tests of biomass vs. invasion, we find a slight decrease in $\rho$ at long library lengths. This could potentially indicate that the time series that we have stitched together are not sufficiently related to be considered part of a single dynamic process.

Bidirectional causal forcing, with stronger effects of planted biomass on invasion than vice-versa, would make sense, as weeding treatments should insure that there should only be moderate effects of invading species on plot-level biomass (e.g. by decreasing biomass of planted species through light competition), while effects of plot-level planted biomass on invader success should be much stronger (e.g. through competition for space or light).

## Apple-Blossom Thrips

Next, we re-examine the classic apple-blossom thrips (*Thrips imaginis*) time series from the Wait Institute in Australia [@Davidson_1948; @Davidson_1948a]. Seasonal outbreaks of thrips were observed to vary greatly in magnitude from year to year, with large outbreaks coinciding across large spatial domains. This led to the hypothesis that regional-scale climatic factors were responsible for controlling the size of the seasonal outbreaks (what might now be called the Moran effect [@Moran_1953]).
...
To test for nonlinearity, we again use S-maps, but test both `E = 4` and `E = 8` to verify that the result is robust to the choice of embedding dimension.
...
The S-map results indicate nonlinearity in *Thrips* abundance, as nonlinear models `theta > 0` give substantially better predictions than the linear model `theta = 0`. This suggests that, despite the strong seasonal dynamics, the *Thrips* population does not simply track the environment. To look more closely at the issue of seasonal drivers, we turn to convergent cross-mapping (CCM).

### Seasonal Drivers

Recall that there is a two-part criterion for CCM to be a rigorous test of causality. (1) The cross map prediction skill is statistically significant when using the full time series as the library. (2) Cross map prediction skill demonstrates convergence, i.e. prediction skill increases as more of the time series is used for the library and the reconstructed attractor becomes more dense.

For an initial summary, we first compute the cross map skill for each variable pair, using the full time series as the library. We use `E = 8` as it is the larger of the two values identified for *Thrips* abundance, and setting a fixed value reduces the chance of overfitting `E` when cross mapping between all variable pairs.
...
However, we are still left with the conundrum that temperature and to a lesser extent, rainfall, are easily predicted from the seasonal cycle. In other words, the cross map results could be an artifact of shared seasonal forcing.

We can distinguish between shared seasonal forcing and a true causal effect if the level of cross mapping obtained for `maxT_degC` and `Rain_mm` is significantly beyond what would be explained by shared seasonality. This hypothesis is readily tested using randomization tests based on surrogate data. The idea here is to generate surrogate time series with the same level of shared seasonality. Cross mapping between the real time series and these surrogates thus generates a null distribution for $\rho$, against which the actual cross map $\rho$ value can be compared.

#### Seasonal Surrogate Test

We use the `make_surrogate_data` function to generate surrogate time series with the same seasonal pattern, but with the anomalies shuffled. Here `T_period` indicates that we want the surrogated to keep the same mean annual signal, corresponding to 12 time steps (since the data are monthly). Next we compute cross mapping between the true abundance and these surrogate environmental variables.
...
These cross map values form a null distribution, allowing us to estimate the $p$ value for rejecting the null hypothesis of mutual seasonality.
...
+ `"P norm"` is the generalization of the L1 and L2 norms which adds a parameter $P$ as the exponent: the distance between a vector $\mathbf{x} = \left( x_1, x_2, \dots, x_n \right)$ and $\mathbf{y} = \left(y_1, y_2, \dots, y_n \right)$ is computed as ${\left((x_1 - y_1)^P + (x_2 - y_2)^P + \dots + (x_n - y_n)^P\right)^{1/P}}$.
* `P`
...
+ the predictions are included as a list-column in the output if enabled
* `exclusion_radius`
...
+ the s-map coefficeints are included as a list-column in the output if enabled
...
Package development and its documentation were supported by a Gordon and Betty Moore Foundation Data-Driven Discovery Initiative through Grant GBMF4563 (to Ethan P. White), U.S. Department of Defense Strategic Environmental Research and Development Program 15 RC-2509 (GS), Lenfest Ocean Program award 00028335 (GS), NSF grant DEB-1020372 (GS), the McQuown Chair in the Natural Sciences (GS), and the Sugihara Family Trust (GS).
