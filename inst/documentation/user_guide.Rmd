...
Precompiled versions can be downloaded from Github [here](https://github.com/ha0ye/rEDM-binary). Please be sure to download the latest version (0.2.4.1 as of July 3, 2015). Clicking on the "view the full file" link should initiate a downlaod of the .tgz or .zip file, which you can save anywhere you like. Note that you do not want to unpack the .tgz or .zip file, since R expects a single package file, rather than a folder.
...
## Source Version
The raw source can be downloaded from Github [here](https://github.com/ha0ye/rEDM). If you are familiar with using Git with Rstudio projects, you can go ahead and clone it directly. Otherwise, it is possible to install it using functions from the **devtools** package:
...
ts <- tentmap_1d
lib <- c(1, 50)
pred <- c(101, length(ts))
simplex_output <- simplex(ts, lib, pred, E = 2, tp = 1:10)
```

Again, the results are a data.frame with columns for the model parameters and forecast statistics, and rows for each run of the model. Here, we can simply plot $tp$ against $\rho$ to see how forecast accuracy changes as we predict further and further into the future:
