...
// use CCM to identify causality
// operates by cross mapping from one variable to another
// must specify E as well as lib portions
// convergene criterion means try different library sizes
// at each library size can randomly sample from vectors
// see help for ccm for more details
...
The output from CCM is a data.frame with statistics for each library used. Because we cross map using multiple libraries at each library size, we'd like to aggregate the results and just plot the mean cross map skill at each library size.
