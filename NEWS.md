...
- `SMap` linear system solver regularization: The R [glmnet](https://cran.r-project.org/web/packages/glmnet/index.html) package does not seperate the model from the data. This prevents use in rEDM. Users requiring `SMap` regularization are referred to the [pyEDM](https://pypi.org/project/pyEDM/) wrapper.
